...
const QuestionPractice = ({ route }) => {
  ...
  const question = useSelector(
    (state) => state.questions.questionPractice.question[index].data.data,
  );
  const visiable = useSelector(
    (state) => state.questions.questionPractice.question[index].visiable,
  );
  const indexQuestion = useSelector(
    (state) => state.questions.questionPractice.question[index].index,
  );
  const score = useSelector(
    (state) => state.questions.questionPractice.question[index].data.score,
  );
  const currentTime = useSelector(
    (state) =>
      state.questions.questionPractice.question[index].data.currentTime,
  );
  ...
  const correctTimes = Object.values(question[indexQuestion]?.answer || {}).map(
    Number,
  );

  //   const questionqw = useSelector(
  //     (state) => state.questions.questionPractice.question[0].data.data,
  //   );
  //   console.log("================================");
  //   console.log(questionqw);
  //   console.log(index);
  //   // console.log(score)
  //   console.log("================================");
  ...
  for (let i = 0; i < correctTimes.length; i++) {
    widthOptions["option" + (i + 1)] =
      (Math.abs(correctTimes[i] - correctTimes[i + 1]) /
  ...
  const loadScore = () => {
    if (score[indexQuestion] >= 0) {
      let count = 5;
      if (
        currentTime[indexQuestion] >= correctTimes[0] &&
        currentTime[indexQuestion] <= correctTimes[correctTimes.length - 1]
      ) {
        for (let i = 0; i < correctTimes.length; i++) {
          if (
            currentTime[indexQuestion] >= correctTimes[i] &&
            currentTime[indexQuestion] <= correctTimes[i + 1]
          ) {
            dispatch(
              setScore({
                value: {
  ...
  const flagPercen = () => {
    const percen =
      (currentTime[indexQuestion] / (status.durationMillis / 1000)) * 88;
    return `${percen}%`;
  };

  const handleNext = async () => {
    dispatch(
      moveToNextQuesionPractice({ value: index, length: question.length }),
    );
    // animatedValue.setValue(0);
  };
          ...
          <View style={styles.container}>
            <View style={styles.question}>
              <View style={styles.headerQuestion}>
                ...
                >{`Câu ${indexQuestion + 1} / ${question.length}`}</Text>
                ...
                {status.positionMillis && status.positionMillis !== 0 ? (
                  <View style={{ flex: 1, backgroundColor: "black" }}>
                    ...
                    <View
                      style={{
                        flex: 1,
                        paddingLeft: flagPercen(),
                        marginBottom: "2%",
                        marginLeft: "5%",
                      }}
                    >
                      {currentTime[indexQuestion] > 0 ? (
            ...
            >
              ...
              <Text
                style={{ fontSize: 18 }}
              >{`${score[indexQuestion] > 0 ? score[indexQuestion] : 0}/5`}</Text>
            </View>

            {!status.isLoaded ||
            (status.positionMillis === 0 && !status.isPlaying) ? (
              <TouchableOpacity
