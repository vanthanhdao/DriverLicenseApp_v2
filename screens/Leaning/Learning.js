...
const Learning = ({ navigation }) => {
  ...
  const totalQuesion = (typeQuestion) => {
    if (question && question.length > 0)
      return question.filter((item) => item.typequestion === typeQuestion);
    return [];
  };

  const completeIQ = useSelector(
    (state) => state.questions.importantQuestion.history,
  );
  const completeRQ = useSelector(
    (state) => state.questions.ruleQuestion.history,
  );
  const compleateQuesion = (history) => {
    if (history && history.length > 0)
      return history.filter((item) => item.style.length > 0);
    return [];
  };
  ...
  const width = [
    `${(compleateQuesion(completeIQ).length / totalQuesion("important").length) * 100}%`,
    `${(compleateQuesion(completeRQ).length / totalQuesion("rule").length) * 100}%`,
    `${(10 / 20) * 100}%`,
    `${(12 / 20) * 100}%`,
    `${(15 / 20) * 100}%`,
    `${(20 / 20) * 100}%`,
  ...
  const completeQ = [
    `${compleateQuesion(completeIQ).length} / ${totalQuesion("important").length}`,
    `${compleateQuesion(completeRQ).length} / ${totalQuesion("rule").length}`,
  ];
  const handleDataTran = (typeQuestion, typeIndex, index) => {
    titleLearning.title = dataItem[index];
    navigation.navigate("Question", {
      typeQuestion:
        typeQuestion && typeQuestion.length > 0 ? typeQuestion[index] : null,
          ...
          {dataItem.map((item, index) => (
            <Surface key={index}>
              ...
              <TouchableOpacity
                ...
                theme={DarkTheme}
                onPress={() => handleDataTran(typeQuestion, typeIndex, index)}
              >
