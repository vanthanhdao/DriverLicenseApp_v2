...
const LearningContent = ({ ...props }) => {
  ...
  const correctValues =
    importantQuestions && importantQuestions.length > 0
      ? importantQuestions[index].answer.correctoption
      : "";
  ...
  const getOptionStyle = (index, option, correctValues) => {
    ...
    newStyles[index].background =
      option === correctValues ? "#009900" : "#FF3333";
    newStyles[index].textColor = "white";
    dispatch(setStyles({ target: typeOptionStyle, value: newStyles }));
  };

  const newData = Array.from(
    { length: importantQuestions.length },
    (_, index) => ({
      index: index,
      style: [],
    }),
  ...
  const renderItem = ({ item, index }) => (
    ...
    >
      <Text style={{ fontSize: 24 }}>
        {item.length === 0 ? index + 1 : item.index + 1}
      </Text>
    </TouchableOpacity>
  );
    ...
    <View style={{ flex: 1 }}>
          ...
          <View style={styles.container}>
            <View style={styles.question}>
              <View style={styles.headerQuestion}>
                <TouchableOpacity
                  onPress={() =>
                    dispatch(
                      moveToPreviousQuestion({ target: typeOptionStyle }),
                    )
                  }
                >
                ...
                >{`Câu ${index + 1} / ${importantQuestions.length}`}</Text>
                <TouchableOpacity
                  onPress={() =>
                    dispatch(
                      moveToNextQuestion({
                        target: typeOptionStyle,
            ...
            <View style={{ flex: 1, flexDirection: "row" }}>
              {importantQuestions &&
                ...
                importantQuestions[index].images &&
                importantQuestions[index].images.map((image, index) => (
                  <Image
                    key={index}
                    source={{ uri: image }}
                    style={{
                ...
                >
                  <View
                    style={{
                      ...styles.idOption,
                      backgroundColor:
                        optionStyles && optionStyles.length > 0
                          ? optionStyles[index].background
                          : "white",
                    }}
                  >
                    <Text
            ...
            {optionStyles.map((item, indexStyle) =>
              item && item.background === "#009900" ? (
                <View
                  style={{ ...styles.explan, display: "flex" }}
                  key={indexStyle}
                >
      ...
      {visiable && (
        <TouchableOpacity
          ...
          onPress={() => dispatch(setVisiable({ target: typeIndex }))}
        >
          <FlatList
            data={data}
